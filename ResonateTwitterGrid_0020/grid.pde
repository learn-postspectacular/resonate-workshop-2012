void initGrid() {
  ...
  for (int y=0; y<RESY; y++) {
    for (int x=0; x<RESX; x++) {
      VerletParticle p=new VerletParticle(new Vec3D(x-RESX/2, y-RESY/2, 0).scaleSelf(GRID_SCALE));
...
void addConnection(VerletParticle from, int toX, int toY) {
  VerletParticle to=physics.particles.get(index(toX, toY));
  physics.addSpring(new VerletSpring(from, to, from.distanceTo(to), 0.1));
}
